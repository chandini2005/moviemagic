...
def booking():
    ...
    if request.method == 'POST':
        ...
        date = request.form.get('date', '2025-07-07')  # Default/fake date

        if not seat or not time:
            flash("Please select at least one seat and a show time.")
    ...
    return render_template('booking_form.html', movie='Kubera')
...
def tickets():
    ...
    user_bookings = [b for b in mock_bookings if b['user'] == session['user']]
